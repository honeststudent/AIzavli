...
bool dfsUtil(PuzzleState* curr, set<vector<vector<int>>>& visited, int MAX_DEPTH) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (isValid(newX, newY)) {
            ...
            if (visited.find(newBoard) == visited.end()) {
                PuzzleState* next = new PuzzleState(newBoard, newX, newY,
                                                    curr->depth + 1, curr);

                if (dfsUtil(next, visited, MAX_DEPTH))
                    return true;
            }
        }
    }
...
void solvePuzzleDFS(vector<vector<int>>& start, int x, int y) {
    ...
    if (!dfsUtil(initial, visited, MAX_DEPTH)) {
        cout << "No solution found (DFS reached depth limit)" << endl;
    }
}
